...
MemStore用于缓存数据修改，通过与HLog搭配使用，提高数据的写入效率。除去手动通过API或命令触发Flush，当到达一定阈值后，HBase也会自动触发MemStore的Flush，将内存中的快照同步到文件中，触发时机主要有如下4种：
...
* RegionServer级别：
  * RegionServer内所有MemStore的大小达到`hbase.regionserver.global.memstore.upperLimit`指定的值时，RegionServer会阻塞更新并强制执行flush，
    直到总MemStore大小低于`hbase.regionserver.global.memstore.lowerLimit`配置值。Flush时，按Region为单位，并以Region中MemStore的总和大小降序执行。
