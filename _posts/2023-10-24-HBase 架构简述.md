...
* 每个MemStore默认需要2MB（`hbase.hregion.memstore.mslab.chunksize`）的堆内存缓冲区，过多的Region会过度占用JVM堆，例表有2个列族，则每个Region有2个Store，若RegionServer维护了1000个Region，则仅MemStore就会占用1000 * 2 * 2MB共约4GB的堆内存。
* 过多的Region可会会频繁的触发MemStore的Flush以及相应HFile的合并，导致HBase性能降低。

### HStore

Store在Region中负责维护一个列族，由一个MemStore和0~n个HFile组成。HStore主要有两个功能：
* 将MemStore中的快照Flush到HFile中；
* 对不断写入的HFile进行合并，避免过多的小文件导致查询IO增加，降低查询效率。
...
MemStore用于缓存数据修改，通过与HLog搭配使用，提高数据的写入效率。除去手动通过API或命令触发Flush，当到达一定阈值后，HBase也会自动触发MemStore的Flush，将内存中的快照同步到文件中，触发时机主要有如下种：
...
随着MemStore不断地Flush，会不断生成对应的HFile，导致文件越来越多